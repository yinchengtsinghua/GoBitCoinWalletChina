...
//translaterror创建一个新的GRPC错误，并为其创建适当的错误代码
//已识别错误。
//
//此功能决不是完整的，应根据其他功能进行扩展
//已知错误。任何没有返回grpc错误的rpc处理程序（带有grpc.errorf）
//应返回此结果。
...
//waddrmgr.iserror很方便，但在
//必须检查基础错误。打开潜在错误
//如果它存在。
	if e, ok := err.(waddrmgr.ManagerError); ok {
//对于这些waddrmgr错误代码，基本错误不是
//需要确定GRPC错误代码。
		switch e.ErrorCode {
case waddrmgr.ErrWrongPassphrase: //公共和私人
			return codes.InvalidArgument
		case waddrmgr.ErrAccountNotFound:
			return codes.NotFound
case waddrmgr.ErrInvalidAccount: //保留帐户
			return codes.InvalidArgument
...
//版本服务器为RPC客户端提供了查询RPC服务器的能力
//版本。
type versionServer struct {
}

//WalletServer为RPC客户端提供钱包服务。
type walletServer struct {
	wallet *wallet.Wallet
}

//loaderserver为RPC客户端提供了加载和关闭钱包的功能，
//以及建立到BTCD共识服务器的RPC连接。
...
//StartVersionService创建VersionService的实现，并
//在GRPC服务器上注册。
...
//StartWalletService创建WalletService的实现，并
//在GRPC服务器上注册。
...
//漏洞：
//-发布前未检查交易是否相关使用
//sendrawtransaction，因此到btcd的连接错误可能导致tx
//
//-一旦上述错误被修复，钱包将需要一种方法来清除无效的
//当数据库中的事务被网络拒绝时，其他
//比花两倍的钱还多。
...
//startwalletloaderservice创建walletloaderservice的实现
//并在GRPC服务器上注册。
